...
class MainWindow(QMainWindow):
    ...
    def __init__(self, settings: QSettings | None = None, probe_fn=probe, download_fn=download,
        ...
        self._thumbnail_cache: collections.OrderedDict[str, QPixmap] = collections.OrderedDict()
    ...
    def retranslate_ui(self) -> None:
        ...
        self.clipboard_notice_label.setText(tr("clipboard.notice"))
        self.clipboard_notice_ok.setText(tr("clipboard.notice_ok"))
        self.clipboard_notice_off.setText(tr("clipboard.notice_off"))
    ...
    def _build_ui(self) -> None:
        ...
        # Jednom: program prati kopirane linkove — korisnik zna zašto reaguje i može to odmah isključiti.
        self.clipboard_notice = QFrame()
        self.clipboard_notice.setObjectName("supportBanner")
        notice = QHBoxLayout(self.clipboard_notice)
        notice.setContentsMargins(14, 8, 10, 8)
        self.clipboard_notice_label = QLabel()
        self.clipboard_notice_label.setWordWrap(True)
        notice.addWidget(self.clipboard_notice_label, 1)
        self.clipboard_notice_ok = QPushButton()
        self.clipboard_notice_ok.clicked.connect(lambda: self._close_clipboard_notice(keep=True))
        notice.addWidget(self.clipboard_notice_ok)
        self.clipboard_notice_off = QPushButton()
        self.clipboard_notice_off.clicked.connect(lambda: self._close_clipboard_notice(keep=False))
        notice.addWidget(self.clipboard_notice_off)
        self.clipboard_notice.hide()
        layout.addWidget(self.clipboard_notice)
    ...
    def _load_settings(self) -> None:
        ...
        if self._watch_clipboard and not self._settings.value("clipboard_notice_seen", False, type=bool):
            self.clipboard_notice.show()
        self._support = support.SupportState(
    ...
    def _close_clipboard_notice(self, keep: bool) -> None:
        ...
        self.clipboard_notice.hide()
        if not keep:
            self.set_watch_clipboard(False)
...
def _signing_works() -> bool:
    """Provjera potpisa radi i u spakovanom programu (bez nje ažuriranje odbija svaki instaler)."""
    try:
        from Cryptodome.PublicKey import ECC

        key = ECC.generate(curve="ed25519")
        data = b'{"version":"0"}'
        release_signing.verify_manifest(data, release_signing.sign(data, key).hex(),
                                        (release_signing.public_key_hex(key),))
        return all(len(bytes.fromhex(public)) == 32 for public in release_signing.PUBLIC_KEYS)
    except Exception:  # noqa: BLE001 - self-test samo javlja da ne radi
        return False
...
def main() -> int:
    ...
    mark_running()
    from .browser import remove_stale_cookie_files

    remove_stale_cookie_files()  # zaostali kolačići od naglog prekida ranijeg pokretanja
